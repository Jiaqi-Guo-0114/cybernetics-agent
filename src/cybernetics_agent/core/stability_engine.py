...
if __name__ == "__main__":
    ...
    engine = StabilityEngine(config, ctx)
    engine.initialize()
    ...
    result = engine.with_retry(flaky_func)
    ...
    try:
        engine.with_retry(permanent_error)
        assert False, "应该抛出异常"
    except RuntimeError as e:
        assert "404" in str(e)
    print("  ✅ 测试 2 通过：永久性错误不重试")
    ...
    assert cb.can_execute() is True  # 半开状态
    cb.record_success()
    cb.record_success()
    cb.record_success()
    ...
    result2 = engine.degrade(primary, [fallback])
    assert result2 == "fallback_result"
    assert engine._degradation_count == 1
    print("  ✅ 测试 4 通过：优雅降级")

    # 测试 5: 状态报告
    status = engine.get_status()
    assert status["stats"]["retry_count"] > 0
    assert status["stats"]["degradation_count"] == 1
    print("  ✅ 测试 5 通过：状态报告")
