...
if __name__ == "__main__":
    ...
    config = {
        ...
        "mode": "automatic",
        "actions": [
            {"trigger": "tool_error_rate > 0.3", "action": "retry", "params": {"delay": 5}},
    ...
    assert fb.name == "feedback_loop"
    assert len(fb._rules) == 2
    print("  ✅ 测试 1 通过：初始化与规则解析")

    # 测试 2: 事件处理与指标更新
    evt1 = CyberneticsEvent.create(
        EventType.TOOL_RESULT,
        "sess_001",
        {"tool_name": "search", "success": False},
    )
    fb.on_event(evt1)
    assert fb._tool_calls == 1
    assert fb._tool_errors == 1

    # 正常调用不影响
    evt2 = CyberneticsEvent.create(
        EventType.TOOL_RESULT,
        "sess_001",
        {"tool_name": "search", "success": True},
    )
    fb.on_event(evt2)
    assert fb._tool_calls == 2
    assert fb._tool_errors == 1
    print("  ✅ 测试 2 通过：事件处理与指标")
    ...
    assert fb._evaluate_trigger("tool_error_rate > 0.3", ctx2) is True
    assert fb._evaluate_trigger("tool_error_rate <= 0.3", ctx2) is False
    assert fb._evaluate_trigger("feedback_depth < 3", ctx2) is True
    print("  ✅ 测试 3 通过：触发条件评估")

    # 测试 4: 自动触发
    # 当前失败率 50%，应该触发 retry
    fb2 = FeedbackLoop(config, ctx)
    fb2.initialize()
    for _ in range(4):
        fb2.on_event(CyberneticsEvent.create(
            EventType.TOOL_RESULT, "s1", {"success": False}
        ))
    # 失败率 = 4/4 = 1.0 > 0.3，应触发
    assert len(fb2.get_actions()) == 1
    assert fb2.get_actions()[0].action_type == "retry"
    print("  ✅ 测试 4 通过：自动触发")

    # 测试 5: 最大深度限制
    fb3 = FeedbackLoop(config, ctx)
    fb3.initialize()
    fb3._current_depth = 3
    fb3._evaluate_rules()  # 应该不触发任何动作
    assert len(fb3.get_actions()) == 0
    print("  ✅ 测试 5 通过：最大深度限制")

    # 测试 6: 状态报告
    status = fb2.get_status()
